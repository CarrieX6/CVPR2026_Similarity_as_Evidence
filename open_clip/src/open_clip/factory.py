...
def load_checkpoint(model, checkpoint_path, strict=True):
    ...
    # position_ids 与 transformers 版本相关：旧 checkpoint 可能无此键，新 BertEmbeddings 会注册该 buffer
    position_id_key = 'text.transformer.embeddings.position_ids'
    _md = model.state_dict()
    if position_id_key in state_dict and position_id_key not in _md:
        del state_dict[position_id_key]
    elif position_id_key not in state_dict and position_id_key in _md:
        state_dict[position_id_key] = _md[position_id_key].clone()
    resize_pos_embed(state_dict, model)
    resize_text_pos_embed(state_dict, model)
    # resize 后模型形状可能变化，再同步一次 position_ids（若仍缺失）
    _md = model.state_dict()
    if position_id_key not in state_dict and position_id_key in _md:
        state_dict[position_id_key] = _md[position_id_key].clone()
