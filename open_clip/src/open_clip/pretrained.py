...
def _open_clip_hf_direct_cache_path(model_id: str, filename: str) -> str:
    safe = model_id.replace("/", "__")
    root = os.environ.get(
        "OPEN_CLIP_HF_DIRECT_CACHE",
        os.path.join(os.path.expanduser("~/.cache"), "open_clip_hf_direct"),
    )
...
def _download_url_to_path(url: str, dest_path: str, timeout: int = 900) -> None:
    """单文件 HTTP(S) 下载，不依赖 huggingface_hub API（仅 resolve 静态直链）。"""
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "open_clip/resolve (research)"
        },
    )
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as source:
            ...
            with open(tmp, "wb") as output:
                with tqdm(total=total, ncols=80, unit="iB", unit_scale=True, desc=os.path.basename(dest_path)) as loop:
                    while True:
...
def download_pretrained_from_hf_via_http_resolve(
        model_id: str,
        filename: str = "open_clip_pytorch_model.bin",
        revision: Union[str, None] = "main",
):
    """
    通过 {base}/{model_id}/resolve/{revision}/{filename} 直链拉取（默认先试 hf-mirror，再 huggingface.co）。
    OPEN_CLIP_HF_URL_BASES：逗号分隔的域名前缀列表，可改写为你内网镜像。
    """
    rev = revision if revision else "main"
    dest = _open_clip_hf_direct_cache_path(model_id, filename)
    if os.path.isfile(dest):
        print(f"[open_clip] Using cached direct-download file: {dest}")
        return dest
    bases_raw = os.environ.get(
        "OPEN_CLIP_HF_URL_BASES",
        "https://hf-mirror.com,https://huggingface.co",
    )
    bases = [b.strip().rstrip("/") for b in bases_raw.split(",") if b.strip()]
    errs = []
    for base in bases:
        url = f"{base}/{model_id}/resolve/{rev}/{filename}"
        try:
            print(f"[open_clip] HTTP resolve download: {url}")
            ...
            if os.path.isfile(dest):
                try:
                    ...
                    pass
    raise RuntimeError(
        "HTTP resolve 仍失败。请用浏览器/手机热点/wget 下载两文件后设置 OPEN_CLIP_HF_LOCAL，或见 scripts/download_biomedclip_weights.sh。\n"
        + "\n".join(errs)
    )
...
def download_pretrained_from_hf(
    ...
    # 离线：在同一目录放置 open_clip_pytorch_model.bin 与 open_clip_config.json
    local_root = os.environ.get("OPEN_CLIP_HF_LOCAL", "").strip()
    if local_root:
        local_root = os.path.abspath(os.path.expanduser(local_root))
        candidate = os.path.join(local_root, filename)
        if os.path.isfile(candidate):
            print(f"[open_clip] Using local HuggingFace bundle file: {candidate}")
            return candidate
        raise FileNotFoundError(
            f"OPEN_CLIP_HF_LOCAL={local_root} 下缺少 {filename}。"
            f"请从镜像站或 HF 下载 microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224 仓库中的 "
            f"open_clip_pytorch_model.bin 与 open_clip_config.json 放到该目录。"
        )

    rev = revision if revision else "main"
    http_only = os.environ.get("OPEN_CLIP_HF_HTTP_ONLY", "").lower() in ("1", "true", "yes")
    if http_only:
        return download_pretrained_from_hf_via_http_resolve(model_id, filename, rev)
    ...
    try:
        return hf_hub_download(
            model_id, filename, revision=revision, cache_dir=cache_dir, **dl_kwargs
        )
    except Exception as e:
        warnings.warn(f"[open_clip] hf_hub_download 失败 ({type(e).__name__}: {e})，改用 HTTP resolve 回退。")
        return download_pretrained_from_hf_via_http_resolve(model_id, filename, rev)
