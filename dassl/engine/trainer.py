...
class TrainerBase:
    ...
    def set_model_mode(self, mode="train", names=None):
        ...
        for name in names:
            if mode == "train":
                self._models[name].train() #prompt_learner 进入训练模式
...
class TrainerX(SimpleTrainer):
    ...
    def forward_backward_vlm(self, batch):
        """
        兼容入口：部分分支把训练拆成 VLM/MEH 两阶段，但具体 trainer 可能只实现 forward_backward。
        物理意义：若没有单独实现 VLM 阶段，就退化为标准监督训练一步。
        """
        return self.forward_backward(batch)

    def forward_backward_MEH(self, batch):
        """
        兼容入口：若 trainer 未实现 MEH 训练阶段，则返回空字典（不参与 loss 统计）。
        """
        return {}
    ...
    def run_epoch(self):
        ...
        for self.batch_idx, batch in enumerate(self.train_loader_x):
            ...
            # 阶段 1: 训练 VLM (每一轮都执行)
            loss_summary_vlm = self.forward_backward_vlm(batch)
            loss_summary_meh = self.forward_backward_MEH(batch)
            combined_summary = {**loss_summary_vlm, **loss_summary_meh}

            if hasattr(self, 'logger') and hasattr(self, 'current_round'):
                        self.logger.log_batch_loss(
                            round_idx=self.current_round,
                            epoch=self.epoch,
                            batch_idx=self.batch_idx,
                            loss_summary=combined_summary
                        )


            batch_time.update(time.time() - end)
            losses.update(combined_summary)
        ...
        # Epoch-based LR 调度（cosine / step / warmup）：每个训练 epoch 末各调一次 scheduler.step()。
        # 勿与 optimizer._step_count 比较：后者为「每 batch 一步」计数，与 last_epoch 单位不一致，
        # 且在旧版 PyTorch 上可能无 _step_count，会导致误报「optimizer not stepped」且调度永不更新。
        ...
        # 每10个epoch打印一次摘要
        if (self.batch_idx + 1) == self.num_batches and (self.epoch + 1) % 10 == 0:
            print(f"\n=== Epoch {self.epoch + 1} Summary ===")
            for name in self.get_model_names():
                if self._optims[name] is not None and self._scheds[name] is not None:
                    oc = getattr(self._optims[name], "_step_count", None)
                    oc_str = oc if oc is not None else "n/a"
                    print(f"{name}: lr={self._optims[name].param_groups[0]['lr']:.6f}, "
                          f"optimizer._step_count={oc_str}, "
                          f"scheduler.last_epoch={self._scheds[name].last_epoch}")
            print("="*40 + "\n")
